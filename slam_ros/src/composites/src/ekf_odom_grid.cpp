...
class Node
{
    ...
    visionFeaturesCoordinator vision;
    mrptCoordinator ekf;
    GridMapCoordinator grid;
...
public:
    ...
    Node() : it_(nh_)
    {
         vel_sub_ = nh_.subscribe<nav_msgs::Odometry>("/segway_rmp_node_front/odom",1,&Node::velCallback, this);
        scan_sub_ = nh_.subscribe<sensor_msgs::LaserScan>("/scan",1,&Node::scanCallback, this);
	arucoCam_ = it_.subscribe("/camera/rgb/image_raw",1, &Node::arucoCamCallback, this);
    }
    ~Node(){}
...
void update(){
	...
	grid.setPose(poseX, poseY, poseTheta);
	kill = ekf.kill;	
}
